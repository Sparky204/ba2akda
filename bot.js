...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = ['Scripted By Sparky','Use code Sparkyy'];
    var i = -1;
    var j = 0;
...
client.on("guildMemberAdd", member => {
  ...
  const channel = sWlc[member.guild.id].channel
    ...
    let welcomer = member.guild.channels.find('system-testing', sChannel);
...
member.guild.fetchInvites().then(guildInvites => {
    ...
    const yumz = member.guild.channels.find("system-testing", `${sChannel}`);
